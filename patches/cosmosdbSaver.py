...
class CosmosDBSaver(BaseCheckpointSaver):
    ...
    def get_tuple(self, config: RunnableConfig) -> Optional[CheckpointTuple]:
        ...
        try:
            checkpoint_data = self.container.read_item(item=checkpoint_key, partition_key=partition_key)
        except Exception:
            checkpoint_data = {}
    ...
    def list(self, config: Optional[RunnableConfig], *, filter: Optional[dict[str, Any]] = None, before: Optional[RunnableConfig] = None, limit: Optional[int] = None) -> Iterator[CheckpointTuple]:
        ...
        query = f"SELECT {top_clause} * FROM c WHERE c.partition_key=@partition_key AND IS_DEFINED(c.checkpoint) ORDER BY c.id DESC"
        parameters = [
            {"name": "@partition_key", "value": partition_key}
        ]
        items = list(self.container.query_items(query=query, parameters=parameters, partition_key=partition_key))
    ...
    def _load_pending_writes(self, thread_id: str, checkpoint_ns: str, checkpoint_id: str) -> List[PendingWrite]:
        ...
        writes = list(self.container.query_items(query=query, parameters=parameters, partition_key=partition_key))
    ...
    def _get_checkpoint_key(self, container, thread_id: str, checkpoint_ns: str, checkpoint_id: Optional[str]) -> Optional[str]:
        ...
        query = "SELECT TOP 1 c.id FROM c WHERE c.partition_key=@partition_key AND IS_DEFINED(c.checkpoint) ORDER BY c.id DESC"
        parameters = [
            {"name": "@partition_key", "value": partition_key}
        ]
        items = list(container.query_items(query=query, parameters=parameters, partition_key=partition_key))
